...
abstract class aiassign_plugin {
    ...
    public final function set_config($name, $value) {
        ...
        $dbparams = array('assignment'=>$this->assignment->get_instance()->id,
            ...
            'name'=>$name);
        $current = $DB->get_record('aiassign_plugin_config', $dbparams, '*', IGNORE_MISSING);

        if ($current) {
            $current->value = $value;
            return $DB->update_record('aiassign_plugin_config', $current);
        } else {
            ...
            return $DB->insert_record('aiassign_plugin_config', $setting) > 0;
        }
    }
    ...
    public final function get_config($setting = null) {
        ...
        if ($setting) {
            ...
            if ($assignment) {
                $dbparams = array('assignment'=>$assignment->id,
                    ...
                    'plugin'=>$this->get_type(),
                    'name'=>$setting);
                $result = $DB->get_record('aiassign_plugin_config', $dbparams, '*', IGNORE_MISSING);
                if ($result) {
                    return $result->value;
                }
            }
        ...
        $dbparams = array('assignment'=>$this->assignment->get_instance()->id,
            'subtype'=>$this->get_subtype(),
            'plugin'=>$this->get_type());
        $results = $DB->get_records('aiassign_plugin_config', $dbparams);
    ...
    public function get_file_info($browser, $filearea, $itemid, $filepath, $filename) {
        ...
        if ($this->get_subtype() == 'assignsubmission') {
            if ($itemid) {
                $record = $DB->get_record('aiassign_submission', array('id' => $itemid), 'userid,groupid', IGNORE_MISSING);
                if (!$record) {
                    return null;
                }
