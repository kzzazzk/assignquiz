...
class aiassign_submission_file extends aiassign_submission_plugin
{
    ...
    private function get_file_submission($submissionid) {
        global $DB;
        return $DB->get_record('aiassignsubmission_file', array('submission'=>$submissionid));
    }
    ...
    public function get_settings(MoodleQuickForm $mform) {
        ...
        if ($this->assignment->has_instance()) {
            ...
            $defaultmaxfilesubmissions = get_config('aiassignsubmission_file', 'maxfiles');
            $defaultmaxsubmissionsizebytes = get_config('aiassignsubmission_file', 'maxbytes');
            $defaultfiletypes = get_config('aiassignsubmission_file', 'filetypes');
        }
    ...
    public function save(stdClass $submission, stdClass $data) {
        ...
        if ($filesubmission) {
            $filesubmission->numfiles = $this->count_files($submission->id,
                ASSIGNSUBMISSION_FILE_FILEAREA);
            $updatestatus = $DB->update_record('aiassignsubmission_file', $filesubmission);
            ...
            $filesubmission->id = $DB->insert_record('aiassignsubmission_file', $filesubmission);
            $params['objectid'] = $filesubmission->id;
    ...
    public function remove(stdClass $submission) {
        ...
        if ($currentsubmission) {
            ...
            $DB->update_record('aiassignsubmission_file', $currentsubmission);
        }
    ...
    public function view_summary(stdClass $submission, & $showviewlink) {
        ...
        if ($count <= ASSIGNSUBMISSION_FILE_MAXSUMMARYFILES) {
            return $this->assignment->render_area_files('aiassignsubmission_file',
                ASSIGNSUBMISSION_FILE_FILEAREA,
                $submission->id);
        } else {
            return get_string('countfiles', 'aiassignsubmission_file', $count);
        }
    }
    ...
    public function upgrade(context $oldcontext,
        ...
        if (!$DB->insert_record('aiassignsubmission_file', $filesubmission) > 0) {
    ...
    public function delete_instance() {
        ...
        $DB->delete_records('aiassignsubmission_file',
    ...
    public function format_for_log(stdClass $submission) {
        ...
        return get_string('numfilesforlog', 'aiassignsubmission_file', $filecount);
    }
    ...
    public function copy_submission(stdClass $sourcesubmission, stdClass $destsubmission) {
        ...
        if ($filesubmission = $this->get_file_submission($sourcesubmission->id)) {
            ...
            $DB->insert_record('aiassignsubmission_file', $filesubmission);
        }
        return true;
    }
    ...
    public function get_config_for_external() {
        ...
        if ($configs->maxsubmissionsizebytes == 0) {
            $configs->maxsubmissionsizebytes = get_max_upload_file_size($CFG->maxbytes, $this->assignment->get_course()->maxbytes,
                get_config('aiassignsubmission_file', 'maxbytes'));
        }
